...
int main() {
    ...
    // 创建根节点
    tree.createRoot(1);
    cout << "Root created: 1" << endl;
    
    // 插入节点
    tree.insert(1, 2, true);   // 2作为1的左子节点
    tree.insert(1, 3, false);  // 3作为1的右子节点
    tree.insert(2, 4, true);   // 4作为2的左子节点
    tree.insert(2, 5, false);  // 5作为2的右子节点
    tree.insert(3, 6, true);   // 6作为3的左子节点
    tree.insert(3, 7, false);  // 7作为3的右子节点
    
    // 打印树结构
    cout << "\nBefore threading:" << endl;
    tree.printTree();
    
    // 线索化二叉树
    tree.makeThreaded();
    cout << "\nAfter threading:" << endl;
    tree.printTree();
    ...
    // 更新节点值
    cout << "\nUpdating node 5 to 50: ";
    cout << (tree.update(5, 50) ? "Success" : "Failed") << endl;
    tree.inOrderTraversal();
    
    // 删除节点
    cout << "\nDeleting node 2: ";
    cout << (tree.remove(2) ? "Success" : "Failed") << endl;
    tree.printTree();
    tree.inOrderTraversal();
    
    cout << "\nDeleting node 1 (root): ";
    cout << (tree.remove(1) ? "Success" : "Failed") << endl;
    tree.printTree();
    
    return 0;
}
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NDYyOTAxMDgsMTEyMDk2MDU1XX0=
-->