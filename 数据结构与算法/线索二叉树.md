# 线索二叉树
## 与二叉树的区别在于将空指针利用起来
左空指针用于指向当前节点的前驱，右空指针用于指向对当前节点的后继，为了区分当前指针指向的是孩子还是前驱后继，在节点的左右指针增加一个标签，1表示指向孩子，0表示指向前驱后继
线索二叉树的优势在于遍历一次后记忆前驱后继，后续求前驱后继时不用再次遍历
```c++
// 线索二叉树节点定义
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMDE1ODgxMzksLTE2OTM0MDc4MTAsMT
EyMDk2MDU1XX0=
-->