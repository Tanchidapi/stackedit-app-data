...
##### 历史未通过说明
1. cpp的动态数组应用vector实现，如vector<int> g(n, 0)可以创建一个长度为n的动态数组，这在n是变量的情况下十分实用，n后面的0表示初始化数组所有值为0
2. c的动态数组通过malloc或者calloc实现，如int *g = (int *)malloc(n * sizeof(int))
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjQ4MTY1NzMsMTczMTUyOTgxNCwtNjU4Nj
kwMzU0XX0=
-->