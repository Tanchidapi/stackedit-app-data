...
### NewReno
NR在超时方面仍然和Tahoe相同，在收到三个重复ACK时也和Reno一样进行快速重传，但是在快速重传时会有一些精妙的操作，NR在每收到一个重复的ACK就会扩展一个窗口，因为其认为能收到ACK，说明数据包正在成功到达接收方，但是不能发送太快，因为出现了重复的ACK，正在接近网络的拥塞点，但是当执行快速重传时，必须等待一个完整的RTT才能发送新数据包
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAzNzc2NzU2MCwtNDM3MjIyNzY1LDE1OD
