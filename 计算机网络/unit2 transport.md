...
上述四个例子中的第四个是回复延迟的情况，这是所有可靠协议都会遇到的一个问题，这种情况下，第二次发送方收到的ack无法确认是哪一数据报对应的，停等协议下通过使用一个一比特计数器解决问题，发送的数据和回传的ack有同样的计数，对于这个比特计数器有两个简单的假设，一是网络不重传数据报，二是数据报不会延迟超过多个超时时长
...
![输入图片说明](/imgs/2025-07-18/MDfIGCkUr64IAmjA.png)滑动窗口发送方的工作
...
go back n协议在多个连续的数据报丢失时效率要高于selective repeat，速度更快，当sws为4，rws为1时，重传的表现类似于go back n，当sws于rws都为4时，重传的表现类似于selective repeat
 ...
 tcp头部biao'zhun
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NTk3ODE3NjAsLTE1NzA2NjE0NDksMz
