...
[[1,4,14],[4,7,16]]
```
差分数组题目，因为每一次染色都是对一段范围内加上同一个数字，因此构造差分数组，只有区间起点和终点对应差分数组的值会改变，此处需注意染色时是对端点内染色，要注意染色线段和区间端点的关系，同时记录本次染色的起终点，更新完差分数组后顺序检查每一个端点，若为1说明是某一段染色线段的起点，从其开始找到下一个为1
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTYwNjMyMjAsMjE0MjczMDQxMiwtMT
gzMzk1OTU0OF19
-->