# 排序
## 1.插入排序
### 1.1直接插入排序
依次将每个元素插入到前面有序的部分，时间复杂度O(n^2)，为稳定排序
```c++
template <typename T>
void insertionSort(vector<T>& arr) {
    int n = arr.size();
    for (int i = 1; i < n; i++) {
        T key = arr[i]; // 当前待插入元素
        int j = i - 1;  // 从当前元素的前一个开始比较
        
        // 将比key大的元素向后移动
        while (j >= 0 && arr[j] > key) {
            arr[j + 1] = arr[j];
            j--;
        }
        
        // 插入元素到正确位置
        arr[j + 1] = key;
    }
}
```
### 1.2折半插入排序
依次将每个元素插入到前面有序的部分，不过查找插入位置的方式变为折半查找，找到插入位置后在统一后移插入，时间复杂度为O(n^2)，稳定排序
```c++
template <typename T, typename Compare = less<T>>
void binaryInsertionSort(vector<T>& arr, Compare comp = Compare()) {
    int n = arr.size();
    
    for (int i = 1; i < n; i++) {
        T key = arr[i];
        int left = 0;
        int right = i - 1;
        
        // 二分查找插入位置
        while (left <= right) {
            int mid = left + (right - left) / 2;
            if (comp(key, arr[mid])) {
                right = mid - 1;
            } else {
                left = mid + 1;
            }
        }
        
        // 移动元素
        for (int j = i - 1; j >= left; j--) {
            arr[j + 1] = arr[j];
        }
        
        // 插入元素
        arr[left] = key;
    }
}
```
### 1.3表插入排序
类似于单链表的结构，每个节点按顺序指向下一个节点，时间复杂度为O(n^2)，为稳定排序
```c++
// 插入节点到有序链表
void sortedInsert(shared_ptr<ListNode<T>>& sortedHead, shared_ptr<ListNode<T>> newNode) {
    // 如果链表为空或新节点应插入头部
    if (!sortedHead || newNode->data <= sortedHead->data) {
        newNode->next = sortedHead;
        sortedHead = newNode;
    } else {
        // 查找插入位置
        auto current = sortedHead;
        while (current->next && current->next->data < newNode->data) {
            current = current->next;
        }
        // 插入新节点
        newNode->next = current->next;
        current->next = newNode;
    }
}

// 执行表插入排序
void sort() {
    if (!head || !head->next) return;
    
    shared_ptr<ListNode<T>> sortedHead = nullptr;
    auto current = head;
    
    while (current) {
        auto nextNode = current->next;
        sortedInsert(sortedHead, current);
        current = nextNode;
    }
    
    head = sortedHead;
}
```
### 1.4希尔排序
设定初始增量d=n/2，以增量为单位进行排序，每次增量除以2，增量为1时为最后一趟排序结束，时间复杂度在O(n ^ 1.3)~O(n ^ 1.5)不等，为不稳定排序
```c++
```
## 2.快速排序
### 2.1冒泡排序
相邻两两不停比较，大的往后移，比较至最后一个元素结束一趟，共进行n-1趟，时间复杂度为O(n^2)，为稳定排序
```c++
// 希尔排序类
class ShellSort {
public:
	// 希尔排序（使用Shell原始序列）
    template <typename T>
    static void shellOriginal(vector<T>& arr) {
        int n = arr.size();
        for (int gap = n / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < n; i++) {
                T temp = arr[i];
                int j;
                for (j = i; j >= gap && arr[j - gap] > temp; j -= gap) {
                    arr[j] = arr[j - gap];
                }
                arr[j] = temp;
            }
        }
    }
}
```
### 2.2快排
设置一个哨兵数，指针从数组左右两端开始移动，左边在遇到大于哨兵数的数时停下，右边在遇到小于哨兵数的数时停下，交换两数位置，直到左指针超过右指针，若左指针未到左端，则左侧继续进行排序，右侧同理，时间复杂度为O(n*log2n)，为不稳定排序
```c++
class QuickSort {
public:
    // 基础递归快速排序 (Lomuto分区)
    template <typename T>
    static void lomutoRecursive(vector<T>& arr, int low, int high) {
        if (low < high) {
            int pi = lomutoPartition(arr, low, high);
            lomutoRecursive(arr, low, pi - 1);
            lomutoRecursive(arr, pi + 1, high);
        }
    }
private:
    // Lomuto分区方案
    template <typename T>
    static int lomutoPartition(vector<T>& arr, int low, int high) {
        T pivot = arr[high]; // 选择最后一个元素作为枢轴
        int i = low - 1;
        
        for (int j = low; j < high; j++) {
            if (arr[j] <= pivot) {
                i++;
                swap(arr[i], arr[j]);
            }
        }
        swap(arr[i + 1], arr[high]);
        return i + 1;
    }
```
## 3.选择排序
### 3.1简单选择排序
从无序区中不停的选择最小的数放到最前面，时间复杂度是O(n^2)，为不稳定排序
```c++
template <typename T>
static void basic(vector<T>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; i++) {
        int min_idx = i;
        for (int j = i + 1; j < n; j++) {
            if (arr[j] < arr[min_idx]) {
                min_idx = j;
            }
        }
        swap(arr[i], arr[min_idx]);
    }
}
```
### 3.2树选择排序
将待排序的序列作为树的一个个叶子节点，两两一组生成子树，选择最小的那个作为根节点，逐层往上构建，选出最小值，然后将其放入排序后的数组后将其变为无穷大，再次进行排序，时间复杂度O(n*log2n)，为稳定排序
```c++
```
### 3.3堆排序
利用大根堆的性质，详见数据结构——堆，的部分，时间复杂度为O(n*log2n)，为不稳定排序
代码略
## 4.归并排序
### 
归并排序的关键在于归并二字，本质上就是对一系列小数组进行合并，基本情况为两个长度为一的数组按大小进行合并，随后得到一个长度为二的数组，两个长度为2的数组再继续进行归并……以此类推，
时间复杂度为O(n*log2n)，为稳定排序
```c++
#include <iostream>
#include <vector>
#include <climits>
#include <cmath>
#include <algorithm>
#include <random>
#include <chrono>
#include <iomanip>
#include <queue>
using namespace std;
using namespace std::chrono;

class TournamentSort {
private:
    // 树节点结构
    struct Node {
        int value;   // 节点值
        int index;   // 在原始数组中的索引
        bool active; // 节点是否活跃

        Node(int v = INT_MAX, int idx = -1, bool act = true)
            : value(v), index(idx), active(act) {}
    };

    // 构建锦标赛树
    static void buildTournamentTree(vector<Node>& tree, const vector<int>& arr) {
        int n = arr.size();
        int treeSize = 2 * n - 1; // 完全二叉树的节点数
        
        // 初始化树结构
        tree.resize(treeSize);
        
        // 设置叶子节点
        for (int i = 0; i < n; i++) {
            tree[n - 1 + i] = Node(arr[i], i);
        }
        
        // 构建非叶子节点
        for (int i = n - 2; i >= 0; i--) {
            int leftChild = 2 * i + 1;
            int rightChild = 2 * i + 2;
            
            // 选择较小的子节点作为父节点
            if (tree[leftChild].value <= tree[rightChild].value) {
                tree[i] = tree[leftChild];
            } else {
                tree[i] = tree[rightChild];
            }
        }
    }

    // 重新调整锦标赛树
    static void replayTournament(vector<Node>& tree, int winnerIdx) {
        int n = (tree.size() + 1) / 2; // 原始元素数量
        int current = n - 1 + winnerIdx; // 叶子节点位置
        
        // 将胜者标记为无效
        tree[current].active = false;
        tree[current].value = INT_MAX;
        
        // 向上调整树
        while (current > 0) {
            int parent = (current - 1) / 2;
            int leftChild = 2 * parent + 1;
            int rightChild = 2 * parent + 2;
            
            // 选择活跃的最小子节点
            if (!tree[leftChild].active || 
                (tree[rightChild].active && tree[rightChild].value < tree[leftChild].value)) {
                tree[parent] = tree[rightChild];
            } else {
                tree[parent] = tree[leftChild];
            }
            
            current = parent;
        }
    }

public:
    // 树形选择排序
    static void sort(vector<int>& arr) {
        if (arr.empty()) return;
        
        int n = arr.size();
        vector<Node> tree;
        
        // 构建锦标赛树
        buildTournamentTree(tree, arr);
        
        // 存储排序结果
        vector<int> sorted;
        sorted.reserve(n);
        
        // 进行n次选择
        for (int i = 0; i < n; i++) {
            // 根节点是当前最小元素
            sorted.push_back(tree[0].value);
            
            // 重新调整树
            replayTournament(tree, tree[0].index);
        }
        
        // 将排序结果复制回原数组
        arr = sorted;
    }

    // 可视化锦标赛树
    static void visualizeTree(const vector<Node>& tree) {
        if (tree.empty()) {
            cout << "Tree is empty!" << endl;
            return;
        }
        
        int levels = 0;
        int nodes = tree.size();
        while (nodes > 0) {
            levels++;
            nodes /= 2;
        }
        
        int index = 0;
        for (int level = 0; level < levels; level++) {
            int nodesInLevel = pow(2, level);
            int indent = pow(2, levels - level) - 1;
            
            // 打印缩进
            for (int i = 0; i < indent; i++) {
                cout << " ";
            }
            
            // 打印当前层节点
            for (int i = 0; i < nodesInLevel && index < tree.size(); i++) {
                if (tree[index].value == INT_MAX) {
                    cout << "∞";
                } else {
                    cout << tree[index].value;
                }
                
                // 打印节点间距
                for (int j = 0; j < 2 * indent + 1; j++) {
                    cout << " ";
                }
                
                index++;
            }
            cout << endl;
        }
    }
};

// 生成测试数据
vector<int> generateTestData(int size, const string& type) {
    vector<int> data(size);
    random_device rd;
    mt19937 gen(rd());
    
    if (type == "random") {
        uniform_int_distribution<int> dis(1, size * 10);
        generate(data.begin(), data.end(), [&]() { return dis(gen); });
    } 
    else if (type == "sorted") {
        iota(data.begin(), data.end(), 0);
    }
    else if (type == "reverse") {
        iota(data.rbegin(), data.rend(), 0);
    }
    else if (type == "nearly") {
        iota(data.begin(), data.end(), 0);
        uniform_int_distribution<int> dis(0, size - 1);
        for (int i = 0; i < size / 10; i++) {
            swap(data[dis(gen)], data[dis(gen)]);
        }
    }
    else if (type == "duplicates") {
        uniform_int_distribution<int> dis(1, 10);
        generate(data.begin(), data.end(), [&]() { return dis(gen); });
    }
    
    return data;
}

// 打印数组
template <typename T>
void printArray(const vector<T>& arr, int limit = 20) {
    int n = min(static_cast<int>(arr.size()), limit);
    cout << "[";
    for (int i = 0; i < n; i++) {
        cout << arr[i];
        if (i < n - 1) cout << ", ";
    }
    if (arr.size() > limit) {
        cout << ", ... (" << arr.size() << " elements)";
    }
    cout << "]" << endl;
}

// 测试排序算法
void testTournamentSort(vector<int> data, bool visualize = false) {
    vector<int> copy = data;
    
    if (visualize) {
        cout << "初始锦标赛树结构:" << endl;
        vector<TournamentSort::Node> tree;
        TournamentSort::buildTournamentTree(tree, copy);
        TournamentSort::visualizeTree(tree);
    }
    
    auto start = high_resolution_clock::now();
    TournamentSort::sort(copy);
    auto end = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(end - start);
    
    bool sorted = is_sorted(copy.begin(), copy.end());
    
    cout << "树形选择排序 | "
         << setw(10) << duration.count() << " μs | "
         << (sorted ? "排序成功" : "排序失败") << endl;
    
    if (visualize) {
        cout << "排序结果: ";
        printArray(copy);
    }
}
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI5NjU5NTMwNCwtNzI5OTc1MTY4LC0xNj
Y1MjMxNzcyLDIwODQzMTU4LC0yMDMwMzMyNTk4LC0yMTMyNjYy
MjY1LDI2NTY2NzI0MCw2MDM0NjE3NjAsNDg1MDY2NTg3LDE4Nz
cwODA3XX0=
-->