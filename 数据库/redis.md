...
AOF通过追加文件，在写命令执行的时候不仅将命令写入内存，还将命令写入到一个追加的文件中，该文件以日志的形式记录每一个写操作，redis重启时就通过重新执行AOF中的命令来在内存中重新构建数据库，开启方法在配置文件中
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjE3ODAxMTQsLTEyMTg5OTU3OTksLTQ1Mj
