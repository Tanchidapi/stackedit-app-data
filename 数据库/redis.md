...
默认配置就是主节点，对从节点，可以通过命令配置或者修改配置文件，使用命令replicaof host port设置从节点，可以用role命令查看节点角色；配置文件修改redis.conf中对应的端口号和pid文件名，再修改dbfilename以区分主从节点，最后修改replicaof相关选项即可
### 哨兵模式
哨兵会以一个独立的进程运行在redis集群中，执行下面几个功能：通过不断的发送命令来监控节点是否正常；如果某个节点出问题会通过发布订阅模式通知其他节点；最后是自动故障转移，主节点不能正常工作时会选择一个从节点升级为新的主节点。通过以下命令配置哨兵模式：
cli中使用redis -sentinel来启动哨兵模式，需要添加配置文件sentinel.conf（具体配置可查看文档或者AI）
哨兵本身作为一个进程也可能故障，因此可以配置多个哨兵来保证集群的稳定
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkzNDgzMDY0MSw5MzI0OTMxNCwtMTIxOD
k5NTc5OSwtNDUyNDIxMDM5LDE2NzczNTU4NzMsMjA1MjkxMjAy
MCwtNDk2MTAzODc4LC0xMTY4Mzc3ODQyLDEyNzczMTQ2OTQsNj
k0Mzk3Mjk0LC01NzY2MzQyMjgsLTg4Njk1NDU3OSwxNjA2NDU2
NjczLC0xMDMyMzc2NDUsNjgzNzM3MzI5LC05NDMwMzMwOTEsLT
IwODg3NDY2MTJdfQ==
-->